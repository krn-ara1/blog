<template>
  <div class="statusMessageContainer">
    <img
      alt="管理人のアイコン"
      class="statusMessageIcon"
      src="/icon_photo.jpg"
      width="1605"
      height="1606"
    />
    <div class="statusMessageTextContainer">
      <div class="statusMessageTextWrapper">
        <span class="statusMessageText">
          このサイトは作成中ですm(__)m
        </span>
      </div>
    </div>
  </div>
</template>

<style scoped>
.statusMessageContainer {
  display: flex;
  align-items: center;
  gap: calc(0.8rem * 0.5);
}
.statusMessageIcon {
  background-color: #ffffff;
  border: 1px solid var(--color-text);
  width: calc(0.8rem * 1.6 + 0.8rem * 1.6 * 0.2 * 2);
  height: calc(0.8rem * 1.6 + 0.8rem * 1.6 * 0.2 * 2);
  border-radius: 50%;
  filter: contrast(0.8);
}
.statusMessageTextContainer {
  background-color: var(--color-secondary);
  border: 1px solid var(--color-text);
  width: calc(0.8rem * 20);
  padding: calc(0.8rem * 0.2) calc(0.8rem * 1);
  border-radius: 99999px;
  overflow: hidden;
}
.statusMessageTextWrapper {
  overflow: hidden;
}
.statusMessageText {
  display: inline-block;
  padding-left: 100%;
  white-space: nowrap;
  animation: slideSample 8s linear infinite;
}
@keyframes slideSample {
  0% {
    transform: translateX(0%)
  }
  100% {
    transform: translateX(-100%);
  }
}
@media screen and (max-width: 768px) {
  .statusMessageTextContainer {
    width: calc(0.8rem * 10);
  }
  .statusMessageText {
    animation: slideSample 4s linear infinite;
  }
}
</style>